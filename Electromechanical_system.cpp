...
int main(int argc, char* argv[]) {
    ...
    adios2::ADIOS adios;
    std::string output_file = "all_groups.bp";
    ...
    for (int i = 0; i < N; ++i) {
      ...
      int status = solver.solve_with_sundials(y0, t_start, t_end, dt, trajectory);
      if (status != 0) {
        std::cerr << "Sundials solver failed for i = " << i << std::endl;
        continue;
      }
